...
"""Convert PyPI entries to Arch Linux packages using python38."""
...
url={pkg.url}
license=({pkg.license})
depends=(python38 {pkg.depends:{pkg.__class__.__name__}})
## EXTRA_DEPENDS ##
...
def _get_readonly_clean_venv():  # "readonly" is an intent, but not enforced.
    ...
    _run_shell(["python3.8", "-mvenv", venv_dir.name])
...
def _run_python(args, **kwargs):
    ...
    return _run_shell(  # args must be a list; str is not supported.
        [f"{venv_dir}/bin/python3.8"] + args, **kwargs)
...
def _get_metadata(name, setup_requires):
    ...
    with TemporaryDirectory() as venv_dir, \
        ...
        script = textwrap.dedent(r"""
            ...
            show_cmd() {{
                python3.8 - "$(pip show -v "$install_name")" <<EOF
...
def _find_installed_name_version(pep503_name, *, ignore_vendored=False):
    parts = (
        ...
        or _run_shell_stdout(
            f"pacman -Q python38-{pep503_name} 2>/dev/null",
            check=False).split())
    if parts:
        ...
        if pkgname.endswith("-git"):
            ...
            if _run_shell(
                ...
                raise PackagingError(
                    ...
                    f"first.")
        if ignore_vendored and pkgname.startswith("python38--"):
            return
        else:
        ...
        if len(candidates) > 1:
            ...
            try:
                canonical, = (
                    ...
                    if candidate.startswith(f"python38-{pep503_name} "))
            except ValueError:
...
class PackageRef:
    def __init__(self, name, *,
        ...
        if subpkg_of:
            pkgname = f"python38--{self.pep503_name}"
            ...
            default = f"python38-{self.pep503_name}", None
...
class Package(_BasePackage):
    def __init__(self, ref, options):
        ...
        self._depends = DependsTuple(
            ...
            # packages...
            NonPyPackageRef("python38-{}".format(pep503_normalize_name(req)))
    ...
    def provides(self):
        ...
        if self._ref.pkgname.startswith("python38--"):
            return ""
        try:
